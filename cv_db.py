...
def page_accueille():
    ...
    st.info(
        " Dans le menu déroulant _***Navigation***_ de la sidebar, choisissez une catégorie pour accéder au contenu."
    )
    ...
    col1.image(glob_comp, use_container_width=True)
    col3.subheader("Expériences")
    col3.image(glob_exp, use_container_width=True)

    col4, col5, col6, col7, col8 = st.beta_columns((3, 0.00000001, 1.1, 0.2, 1.4))
    col4.subheader("Intérêts")
    col4.image(glob_int, use_container_width=True)
    col6.subheader("Formations")
    col6.image(glob_form, use_container_width=True)
    col8.subheader("Keywords")
    col8.image(glob_kw2, use_container_width=True)
...
def page_competence():
    ...
    st.text("")
    st.text("")
    st.image(comp_duo, use_container_width=True)  # width=600)
    st.image(comp_ico, use_container_width=True)  # width=600)
...
def page_formation():
    ...
    st.text("")
    st.text("")
    st.image(form_road, use_container_width=True)  # ,width=900)
...
def page_experience():
    ...
    st.text("")
    st.markdown(
        "<h5 style='text-align: center; color: gray;'>Choisissez une catégorie</h5>",
        unsafe_allow_html=True,
    )
    st.text("")
    col1, col2, col3 = st.beta_columns(3)
    col1.image(exp_data, use_container_width=True)
    col2.image(exp_chimie, use_container_width=True)
    col3.image(exp_open, use_container_width=True)

    col4, col5, col6, col7, col8, col9 = st.beta_columns(
        (0.5, 1, 0.5, 1, 0.5, 1)
    )  # ((0.5,1,0.35,1,0.45,1))
    ...
    if bouton1:
        ...
        col13.write(
            "La volonté de partir en formation Data Scientist vient du constat, fait lors de mes diverses expériences professionnelles, que les datas ont investi tous les cœurs de métier. La pertinence et la puissance d’utiliser ces datas pour comprendre et prédire des comportements sont apparues comme une suite logique à mon parcours professionnel et ainsi acquérir un nouvel outil polyvalent."
        )
        col14.image(exp_data_oc, use_container_width=True)

        st.subheader("Missions")
        st.write(
            "La formation se décline sous forme de 7 projets professionnels utilisant des données terrain. L’environnement mis en place pour ces projets est le language Python et la méthodologie de travail appliquée est la méthode S.M.A.R.T."
        )
        st.text("")
        ...
        col15.write(
            "**_Anticipation de la consommation et des émissions de bâtiments_** "
        )
        col15.write(
        ...
        col17.write(
            "Mise en place d’un outil de classification automatique de produits de consommation en fonction de leurs descriptifs (texte) et de leurs photos (visuel)."
        )
        col18.write(
            "**_Implémentation d’une métrique métier pour le secteur bancaire_** "
        )
        col18.write(
        ...
        st.write(
            ":black_small_square: Pre-processing, feature engineering, réduction dimensionnelle (ACP)"
        )
        st.write(
            ":black_small_square: Machine learning supervisé : régression, kNN, SVM,  Random Forest, XGBoost, LightGBM"
        )
        st.write(
            ":black_small_square: Machine learning non supervisé : Kmeans, DBScan, HAC, GMM"
        )
        st.write(
            ":black_small_square: Métrique métier, optimisation bayésienne, stacking"
        )
        st.write(
            ":black_small_square: Deep Learning, NLP (BoW, Tf-IDF, Embedding), visuel (BoVW, CNN, Transfert learning)"
        )
        st.write(":black_small_square: Spark, AWS")
        st.write(":black_small_square: Déploiement  API, Dashboard ")
        st.write(
            ":black_small_square: Analyse de résultats, reporting et vulgarisation "
        )
        st.write(
            ":black_small_square: GitHub rassemblant tous les projets (Voir **Média**) "
        )
    ...
    if bouton2:
        ...
        col14.image(exp_sc_broyeur, use_container_width=True)
        ...
        st.subheader("Réalisations")
        st.write(
            ":black_small_square: Synthèse par broyeur planétaire, presse, traitement thermique sous H2, Extraction chimique"
        )
        st.write(
            ":black_small_square: DRX, MEB, Granulométrie, Bobine Supra, ATG, EPMA"
        )
        st.write(
            ":black_small_square: Rédaction du mode opératoire de production (Enveloppe SOLEAU)"
        )
        st.write(
            ":black_small_square: Réalisation des procédures de manipulation et de sécurité"
        )
        ...
        col14.image(exp_sc_four, use_container_width=True)
        ...
        col17, col18 = st.beta_columns((1, 1))
        col17.write(
            "**_Gestion de projets, réunions, plan d'expérience, cahier des charges_** "
        )
        col18.write("**_Conception bancs essai et outils techniques, maintenance_** ")

        st.subheader("Réalisations")
        st.write(
            ":black_small_square: Elaboration d’alliages en four à induction – Frittages. Métallurgie, Hydrure sous pression "
        )
        st.write(":black_small_square: DRX, MEB, EDX, EPMA, DSC, mesure PCT, cyclage")
        st.write(
            ":black_small_square: Modélisation des lois de prédiction de volume de maille d'un alliage quinaire"
        )
        st.write(
            ":black_small_square: Réalisation d'un abaque PCT sur l'utilisation d'Al et Si dans un système TiVFe"
        )
        st.write(":black_small_square: Procédure d'élaboration optimisée")
        st.write(
            ":black_small_square: Manuscrit de thèse - Rapport sur les Energies (Voir **Publications**)"
        )
        st.write(
            ":black_small_square: **1er Prix Présentation de thèse** (Voir **Publications**)"
        )
        ...
        col16.write(
            "**_Réalisation de nano-particules par ablation laser (ns / ps)_** "
        )
        ...
        st.write(
            ":black_small_square: Résultats envoyés pour étude toxicologique (suite du projet) "
        )
        ...
        col13.subheader("Contexte")
        col13.write(
            "Le Projet ROPAS visait la production de batteries flexibles imprimées sur papier. Afin de s’affranchir des constituants classiques et coûteux d’un accumulateur et répondre à l’appauvrissement du lithium, le projet avait pour objectif la réalisation de nouveaux matériaux d’électrodes organiques (ORB) pour accumulateur fonctionnant aussi bien au Lithium, qu’au Sodium."
        )
        col14.image(exp_sc_bat, use_container_width=True)

        st.subheader("Missions")
        st.text("")
        col15, col16 = st.beta_columns((1, 1))
        col15.write(
            "**_Synthèse organique et polymérisation sous atmosphère inerte_** "
        )
        ...
        st.subheader("Réalisations")
        st.write(
            ":black_small_square: Synthèse organique et polymérisation sous atmosphère inerte (Boîte à gants)"
        )
        st.write(":black_small_square: Formulation et mise en œuvre d’encre")
        st.write(
            ":black_small_square: RMN, IR,ATD, ATG, Voltampérométrie, Cyclage d’accumulateur"
        )
        ...
        col13.subheader("Contexte")
        col13.write(
            "Dans une volonté d’ouverture à l’International, cette expérience professionnelle s’est tenue à **Singapour** à un poste de technicien sur chaîne de production d'enzyme pour le textile."
        )
    ...
    if bouton3:
        ...
        col13.subheader("Contexte")
        col13.write(
            "Suite à mon CDD, je suis devenu propriétaire d'une maison datant des années 1930. Nécessitant de gros travaux d'aménagements et de mises à niveau, je me suis lancé dans sa rénovation complète."
        )
        col14.image(exp_diver_maison, use_container_width=True)
        ...
        st.write(
            ":black_small_square: Lancement chaîne YouTube sur le travail du bois (Voir **Média**)"
        )
        ...
        col13.subheader("Contexte")
        col13.write(
            "Opportunité de découvrir le **Japon** tout en travaillant dans un centre d'activités."
        )
        col14.image(exp_diver_jap, use_container_width=True)
...
def page_interet():
    st.markdown("""<a id="top"></a>""", unsafe_allow_html=True)
    st.header("Intérêts")
    st.image(interet_mot, use_container_width=True)
...
def page_media():
    ...
    st.markdown(
        "<h5 style='text-align: center; color: gray;'>Cliquez sur un média pour accéder au contenu</h5>",
        unsafe_allow_html=True,
    )
    st.text("")
    st.text("")

    col4, col5, col6, col7, col8, col9, col10 = st.beta_columns(
        (0.5, 1, 0.5, 1, 0.5, 1, 0.5)
    )
    col5.image(media_cv.resize((125, 150)), use_container_width=True)  # ,width=125)
    col7.image(media_lm.resize((125, 150)), use_container_width=True)  # ,width=125)
    col9.image(media_lr.resize((125, 150)), use_container_width=True)  # ,width=125)
    ...
    col7.image(media_linkedin, use_container_width=True)
    ...
    st.text("")
    st.markdown(
        "<h5 style='text-align: center; color: gray;'>Attention, le son est automatiquement au maximum</h5>",
        unsafe_allow_html=True,
    )
    st.video(media_yt)
...
def page_publi():
    ...
    st.markdown(
        "<h5 style='text-align: center; color: gray;'>Cliquez sur un lien pour accéder au contenu</h5>",
        unsafe_allow_html=True,
    )
    st.text("")
    st.text("")
    ...
    col4, col5, col6, col7, col8, col9, col10 = st.beta_columns(
        (0.5, 1, 0.5, 1, 0.5, 1, 0.5)
    )  # ((0.6,1,0.9,1,0.9,1,0.9,1,0.0001))
    col5.image(publi_brevet, use_container_width=True)
    col7.image(publi_poster, use_container_width=True)
    col9.image(publi_these, use_container_width=True)
    ...
    col5.write(
        "_Polymère comme matériau d'électrode pour des batteries secondaires au lithium._ Réf : **WO 2013156899 A1**"
    )
    col7.write(
        "_Influence of impurities on the performance of metal hydride_. MH2014, Manchester, UK, 2014."
    )
    col9.write(
        "_Étude de l'influence d'éléments d'addition sur les propriétés de stockage de l'hydrogène dans le système Ti-V-Fe._"
    )
    ...
    st.text("")
    col1, col2, col3 = st.beta_columns(3)
    col2.image(publi_prix, use_container_width=True)  # width=220)
    col2.write(pr)
...
st.sidebar.image(bar_img, use_container_width=True)
st.sidebar.markdown(
